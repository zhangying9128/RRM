...
class ZyGeprAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        #zhangying
        agent.add_argument('-use_zy', '--use_zy', type=float, default=0,
                           help='whether use zy method')
        ...
        agent.add_argument('-opt', '--optimizer', default='sgd',
                           choices=ZyGeprAgent.OPTIM_OPTS.keys(),
        ...
        ZyGeprAgent.dictionary_class().add_cmdline_args(argparser)
        return agent
    ...
    def __init__(self, opt, shared=None):
        ...
        if 'train' in opt.get('datatype', ''):
            ...
            # set up optimizer
            lr = opt['learningrate']
            optim_class = ZyGeprAgent.OPTIM_OPTS[opt['optimizer']]
            kwargs = {'lr': lr}
    ...
    def predict(self, xs, ys=None, alpha_matrix=None, profiles=None, persona_mask=None, cands=None, valid_cands=None, is_training=False):
        ...
        if is_training:
            ...
            out = None
            try:
                out = self.model(xs, ys, alpha_matrix, profiles, persona_mask, rank_during_training=cands is not None)
                # generated response
                _preds, scores, scores_q, cand_preds = out[0], out[1], out[2], out[3]
                ...
                if ys is not None and self.opt['use_zy']:
                    #zhangying
                    q_mask = ys != 0  # BxL
                    i_mask = xs != 0  # BxL
                    batch_size = ys.size()[0]
                    
                    i_length = i_mask.sum(dim=1).float().view(-1,1,1)  #Bx1x1
                    t_length = q_mask.sum(dim=1).float().view(-1,1,1)  #BX1X1

                    q_mask = q_mask.unsqueeze(2).float() # BxLx1
                    i_mask = i_mask.unsqueeze(2).float() # BxLx1
                   
                    q_batch = scores_q.transpose(1,2) # BxVocab_sizexL
                    embed_inputs = self.model.encoder.lt(xs).transpose(1,2) #BxEmbed_sizexL
                    ...
                    vi_embedding_matrix = self.model.encoder.lt(source_vocab).view(len(self.dict),-1) #Vocab_sizexEmbed_size
                    ...
                    loss_cos = self.criterion_cos(average_embed_i.squeeze(1), average_q.squeeze(1), sets) / batch_size
                    loss += loss_cos
            ...
            self.model.eval()
            out = self.model(xs, ys=None, alpha_matrix=alpha_matrix, profiles=profiles, persona_mask=persona_mask,cands=cands, valid_cands=valid_cands, beam_size=self.beam_size, topk=self.topk)
            predictions, cand_preds = out[0], out[3]

            if ys is not None:
                # calculate loss on targets
                out = self.model(xs, ys, alpha_matrix, profiles, persona_mask)
                scores = out[1]
                score_view = scores.view(-1, scores.size(-1))
                loss = self.criterion(score_view, ys.view(-1))
    ...
    def vectorize(self, observations):
        ...
        xs, ys, alpha_matrix, profiles, persona_mask, labels, valid_inds, _, _ = PaddingUtils.pad_text( #zhangying
        ...
        if self.use_cuda:
            ...
            persona_mask = persona_mask.cuda()
            if ys is not None:
                ys = ys.cuda()
        ...
        if not is_training and self.rank:
            ...
            for i, v in enumerate(valid_inds):
                if 'label_candidates' in observations[v]:
                    ...
                    cs, _, _, _, _, _, valid_c_inds, *_ = PaddingUtils.pad_text(curr_cands, self.dict, null_idx=self.NULL_IDX, dq=True, truncate=self.truncate)
                    valid_cands.append((i, v, [curr_lcs[j] for j in valid_c_inds]))
                    cs = torch.LongTensor(cs)
                    if self.use_cuda:
                        cs = cs.cuda()
                    cands.append(cs)
        #print(alpha_matrix.size(), profiles.size())
        return xs, ys, alpha_matrix, profiles, persona_mask, labels, valid_inds, cands, valid_cands, is_training
    ...
    def batch_act(self, observations):
        ...
        xs, ys, alpha_matrix, profiles, persona_mask, labels, valid_inds, cands, valid_cands, is_training = self.vectorize(observations)
        ...
        predictions, cand_preds = self.predict(xs, ys, alpha_matrix, profiles, persona_mask, cands, cand_inds, is_training)
...
class PerplexityEvaluatorAgent(ZyGeprAgent):
    ...
    def next_word_probability(self, partial_out):
        ...
        xs, ys, alpha_matrix, profiles, persona_mask = batch[0], batch[1], batch[2], batch[3], batch[4]
        ...
        scores, self.prev_enc = out[1], out[5]
