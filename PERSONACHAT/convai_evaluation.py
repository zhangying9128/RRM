...
from projects.convai2.eval_wordstat import eval_wordstat, setup_args as setup_args_wordstat
...
from train import build_input_from_segments, add_special_tokens_, pad_dataset
from utils import download_pretrained_model, AttrDict
from interact import sample_sequence, top_filtering
...
class TransformerAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        agent_args.add_argument("--eval_type", type=str, default="hits@1", help="hits@1, wordstat, ppl or f1")
        ...
        agent_args.add_argument('--inference', type=str, default="beam", help="greedy, beam or nucleus")
        agent_args.add_argument('--beam_size', type=int, default=4)
        agent_args.add_argument('--alpha', type=float, default=0.95)
        agent_args.add_argument('--block_trigram', action='store_true')
        #zhangying
        #add parameters for deciding test set and output file name
        agent_args.add_argument('--datatype', type=str, default="test", choices=['test', 'valid'], help="do evaluation on the test/valid set")
        agent_args.add_argument('--save_name', type=str, default="test.txt", help="name for output file, only work when eval_type is wordstat")
        agent_args.add_argument('--SEQ2SEQ', action='store_true')
    ...
    def __init__(self, opt, shared=None):
        ...
        self.logger.info('Used parameters for training')
        self.logger.info(torch.load(args.model_checkpoint+'model_training_args.bin'))
            ...
            for subtext in text.split('\n'):
                ...
                if subtext.startswith('your persona:'):
                    ...
                    if not self.args.SEQ2SEQ:
                        ...
                        self.persona.append(self.tokenizer.encode(subtext)[1:-1])
                else:
                    if not self.args.SEQ2SEQ:
                        self.history.append(self.tokenizer.encode(subtext))
                    else:
                        self.history.append(self.tokenizer.encode(subtext)[1:-1])
        ...
        if 'label_candidates' in observation:
            for candidate in observation['label_candidates']:
                if not self.args.SEQ2SEQ:
                    ...
                    candidates.append((self.tokenizer.encode(candidate)[1:-1], candidate))
    ...
    def act(self):
        ...
        if self.args.eval_type == "hits@1" and len(self.candidates) > 0:
            ...
            for candidate, _ in self.candidates:
                instance = build_input_from_segments(self.persona, self.history, candidate, self.tokenizer, SEQ2SEQ=self.args.SEQ2SEQ)
    ...
    def next_word_probability(self, partial_out):
        ...
        if not self.args.SEQ2SEQ:
            ...
            instance = build_input_from_segments(self.persona, self.history, partial_out_ids, 
                                                 self.tokenizer, with_eos=False, SEQ2SEQ=self.args.SEQ2SEQ)
        else:
            partial_out_ids = self.tokenizer.encode(' '.join(partial_out))[1:-1]
            instance = build_input_from_segments(self.persona, self.history, partial_out_ids,
                                                 self.tokenizer, SEQ2SEQ=self.args.SEQ2SEQ, lm_labels=True)
        ...
        if not self.args.SEQ2SEQ:
            ...
            #seq2seq
            input_ids = torch.tensor(instance["input_ids"], device=self.args.device).unsqueeze(0)
            target_ids = torch.tensor(instance["target_ids"], device=self.args.device).unsqueeze(0)
            input_type_ids = torch.tensor(instance["input_type_ids"], device=self.args.device).unsqueeze(0)
            target_type_ids = torch.tensor(instance["target_type_ids"], device=self.args.device).unsqueeze(0)

        with torch.no_grad():
            if not self.args.SEQ2SEQ:
                logits = self.model_checkpoint(input_ids, token_type_ids=token_type_ids)
            else:
                logits = self.model_checkpoint(input_ids, input_ids!=self.tokenizer.convert_tokens_to_ids("<pad>"), target_ids, target_ids!=self.tokenizer.convert_tokens_to_ids("<pad>"), return_dict=False,
                )
...
if __name__ == '__main__':
    ...
    elif opt['eval_type'] == "wordstat":
        setup_args = setup_args_wordstat(None)
        setup_args.set_params(
            dump_predictions_path=opt['model_checkpoint']+'/'+opt['save_name'].strip('\r'))
        setup_args.set_params(
            dict_file='ParlAI/data/models/convai2/dict_self',
            datatype=opt['datatype']) 
        eval_fct = eval_wordstat
    else:
        raise ValueError
