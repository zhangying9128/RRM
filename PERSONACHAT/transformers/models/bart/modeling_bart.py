...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        #zhangying
        self.generator_q = nn.Parameter(torch.Tensor(self.model.shared.num_embeddings, config.d_model)) #from right dim to left dim
        self.generator_q.data.normal_(mean=0.0, std=0.02)
    ...
    def forward(
        ...
        if not return_dict:
            output = (lm_logits,) + (None, ) + outputs[1:]
            return ((masked_lm_loss,) + (0,) + output) if masked_lm_loss is not None else output
