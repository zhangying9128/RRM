...
class Seq2seq(nn.Module):
    ...
    def forward(self, xs, ys=None, alpha_matrix=None, profiles=None, persona_mask=None, cands=None, valid_cands=None, prev_enc=None,
        ...
        alpha_matrix -- batch_size x profile_size x each_line_length
        profiles -- batch_size x profile_size x each_line_length
        ...
        if prev_enc is not None:
            ...
            if profiles is not None:
                encoded_profile = torch.matmul(alpha_matrix.unsqueeze(2), self.encoder.lt(profiles)).squeeze(2)
                #encoded_profile = encoded_profile.sum(1) #batch_size x emb_size
                enc_out = torch.cat((enc_out, encoded_profile),1)
                attn_mask = torch.cat((attn_mask, persona_mask),1)
        ...
        if ys is not None:
            ...
            if self.attn_type == 'none':
                preds, score, q, hidden = self.decoder(xs, hidden, enc_out, attn_mask)
                ...
                for i in range(ys.size(1)):
                    xi = xs.select(1, i)
                    preds, score, q, hidden = self.decoder(xi, hidden, enc_out, attn_mask)
                    predictions.append(preds)
                    scores.append(score)
                    qs.append(q)
        else:
            ...
            if beam_size == 1:
                ...
                for _ in range(self.longest_label):
                    ...
                    preds, score, q, hidden = self.decoder(xs, hidden, enc_out, attn_mask, topk)
                    scores.append(score)
                    xs = preds
                    predictions.append(preds)
        ...
        if qs != []:
            qs = torch.cat(qs, 1)
        return predictions, scores, qs, cand_preds, cand_scores, encoder_states, nbest_beam_preds, nbest_beam_scores
...
class Decoder(nn.Module):
    def __init__(self, num_features, padding_idx=0, rnn_class='lstm',
        ...
        #zhangying
        self.e2q = nn.Linear(emb_size, num_features, bias=True)
    ...
    def forward(self, xs, hidden, encoder_output, attn_mask=None, topk=1):        
        ...
        return preds, scores, scores_q, new_hidden
...
class Ranker(object):
    ...
    def forward(self, cands, cand_inds, decode_params):
        ...
        for i in range(len(cands)):
            ...
            if self.attn_type == 'none':
                preds, score, q, cur_hid = self.decoder(xs, cur_hid, cur_enc, cur_mask)
                ...
                for i in range(curr_cs.size(1)):
                    ...
                    preds, score, q, cur_hid = self.decoder(xi, cur_hid, cur_enc, cur_mask)
                    true_score = F.log_softmax(score, dim=2).gather(
