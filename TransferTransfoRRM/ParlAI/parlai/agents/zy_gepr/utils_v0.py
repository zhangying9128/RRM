...
class PaddingUtils(object):
    ...
    def pad_text(cls, observations, dictionary,
        ...
        parsed_profile = [] #zhangying
        max_pro_len = 0
        max_pro = 0
        if any(['text2vec' in ex for ex in exs]):
            parsed_x = [ex['text2vec'][-1] for ex in exs]
            #zhangying
            for ex in exs:
                persona = list(ex['text2vec'])[:-1]
                #print(persona)
                parsed_profile.append(persona)
                if len(persona) > max_pro:
                    max_pro = len(persona)
                for row in persona:
                    if len(row) > max_pro_len:
                        max_pro_len = len(row)  
        else:
        ...
        parsed_profile = [parsed_profile[k] for k in ind_sorted] if parsed_profile!= [] else []
        ...
        persona_mask = []
        if parsed_profile != []:
            for persona in parsed_profile:
                temp_persona = []
                for row in persona:
                    if len(row) < max_pro_len:
                        temp_persona.append(row + [9999999] * (max_pro_len - len(row)))
                    else:
                        temp_persona.append(row)
                persona_mask.append([1]*len(persona)+[0]*(max_pro-len(persona)))
                if len(persona) < max_pro:
                    for times in range(max_pro-len(persona)):
                        temp_persona.append([9999999] * max_pro_len)
                adjust_profile.append(temp_persona)
    
            adjust_profile = torch.FloatTensor(adjust_profile)
            persona_mask = torch.FloatTensor(persona_mask)
            profile_mask = (adjust_profile != 9999999).float()
            tf_matrix = 1e6 / (adjust_profile ** (1.07))
            alpha_matrix = 1 / (1 + torch.log(1+tf_matrix))
            alpha_matrix = torch.where(profile_mask>0,alpha_matrix,profile_mask)
            adjust_profile = torch.where(profile_mask>0,adjust_profile,profile_mask).long()
            #print(persona_mask)
        ...
        return xs, ys, alpha_matrix, adjust_profile, persona_mask, labels, valid_inds, end_idxs, y_lens
