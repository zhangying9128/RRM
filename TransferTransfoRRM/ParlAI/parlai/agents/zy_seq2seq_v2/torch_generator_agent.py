...
class TorchGeneratorModel(nn.Module):
    ...
    def decode_greedy(self, encoder_states, bsz, maxlen):
        ...
        for i in range(maxlen):
            ...
            scores = scores[:, -1:, :]
            scores, q = self.output(scores)
            _, preds = scores.max(dim=-1)
            logits.append(scores)
            qs.append(q) #zhangying
    ...
    def decode_forced(self, encoder_states, ys):
        ...
        logits, qs = self.output(latent)
        _, preds = logits.max(dim=2)
        return logits, qs, preds
    ...
    def forward(self, *xs, ys=None, cand_params=None, prev_enc=None, maxlen=None,
        ...
        if ys is not None:
            # use teacher forcing
            scores, qs, preds = self.decode_forced(encoder_states, ys)
        else:
            scores, qs, preds = self.decode_greedy(
                encoder_states,
                bsz,
                maxlen or self.longest_label
            )

        return scores, qs, preds, encoder_states
...
class TorchGeneratorAgent(TorchAgent):
    ...
    def compute_loss(self, batch, return_output=False):
        ...
        scores, scores_q, preds, *_ = model_output
        score_view = scores.view(-1, scores.size(-1))
        loss = self.criterion(score_view, batch.label_vec.view(-1))
        if batch.label_lengths is not None:
            #zhangying
            q_mask = batch.label_vec != 0  # BxL
            i_mask = batch.text_vec != 0  # BxL
            batch_size = batch.text_vec.size()[0]
            
            i_length = batch.text_vec.new_tensor(batch.text_lengths).float().view(-1,1,1)  #Bx1x1
            t_length = batch.label_vec.new_tensor(batch.label_lengths).float().view(-1,1,1)  #BX1X1
        
            q_mask = q_mask.unsqueeze(2).float() # BxLx1
            i_mask = i_mask.unsqueeze(2).float() # BxLx1
           
            q_batch = scores_q.transpose(1,2) # BxVocab_sizexL
            embed_inputs = self.model.encoder.lt(batch.text_vec).transpose(1,2) #BxEmbed_sizexL
            ...
            source_vocab = batch.text_vec.new_tensor(range(len(self.dict))).view(1,-1)
    ...
    def eval_step(self, batch):
        ...
        if self.skip_generation:
            ...
            # greedy decode
            _, _, preds, *_ = self.model(*self._model_input(batch), bsz=bsz)
        elif self.beam_size > 1:
