...
class ZySeq2seqAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        agent.add_argument('-opt', '--optimizer', default='sgd',
                           choices=ZySeq2seqAgent.OPTIM_OPTS.keys(),
    ...
    def __init__(self, opt, shared=None):
        ...
        if shared:
            ...
            if opt.get('dict_tokenizer') == 'bpe' and opt['embedding_type'] != 'random':
                ...
                try:
                    ...
                    raise ex
                pretrained_dim = 300
                if opt['embedding_type'].startswith('glove'):    #zhangying Glove index for generateive profile memory network model
        ...
        if 'train' in opt.get('datatype', ''):
            ...
            # set up optimizer
            lr = opt['learningrate']
            optim_class = ZySeq2seqAgent.OPTIM_OPTS[opt['optimizer']]
            kwargs = {'lr': lr}
...
class PerplexityEvaluatorAgent(ZySeq2seqAgent):
