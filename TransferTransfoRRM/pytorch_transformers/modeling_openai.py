...
class OpenAIGPTDoubleHeadsModel(OpenAIGPTPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        #zhangying
        if RRM_METHOD == 'divide':
            self.generator_q_part1 = nn.Parameter(torch.Tensor(config.vocab_size+5, config.n_embd)) 
            self.generator_q_part2 = nn.Parameter(torch.Tensor(config.vocab_size+5, config.n_embd)) 
            self.generator_q_part3 = nn.Parameter(torch.Tensor(config.vocab_size+5, config.n_embd))
            self.generator_q_part1.data.normal_(mean=0.0, std=self.config.initializer_range)
            self.generator_q_part2.data.normal_(mean=0.0, std=self.config.initializer_range)
            self.generator_q_part3.data.normal_(mean=0.0, std=self.config.initializer_range)
        else:
            self.generator_q = nn.Parameter(torch.Tensor(config.vocab_size+5, config.n_embd))
            self.generator_q.data.normal_(mean=0.0, std=self.config.initializer_range)
        self.criterion_cos_none = nn.CosineEmbeddingLoss(reduction='none')
