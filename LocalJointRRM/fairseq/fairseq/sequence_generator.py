    ...
    def generate(
        ...
        **kwargs
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            lprobs, avg_attn_scores, q = model.forward_decoder(
            ...
            # finalize hypotheses that end in eos (except for cands_to_ignore ed ones)
            ...
            # cands_to_ignore ed hypos and values < cand_size indicate candidate
...
class EnsembleModel(torch.nn.Module):
    ...
    def reset_incremental_state(self):
        if self.incremental_states is not None:
            if all(isinstance(m.decoder, FairseqIncrementalDecoder) for m in self.models):
                self.incremental_states = {m: {} for m in self.models}            
